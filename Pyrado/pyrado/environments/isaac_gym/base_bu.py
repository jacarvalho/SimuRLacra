...
class IsaacSimEnv(SimEnv, Serializable):
    """
    ...
    def cleanup(self):
        ...
        self._gym.destroy_sim(self._impl)
    ...
    def step(self, act: np.ndarray) -> tuple:
        ...
        # set targets
        # targets = np.zeros(self.act_space.shape).astype(float)
        # self._gym.set_actor_dof_position_targets(self._envs, self._actors, targets)  # [m] or [rad]

        # apply efforts (every frame)
        # efforts = np.full(self.act_space.shape, 100.0).astype(float)
        # self._gym.apply_actor_dof_efforts(self._envs, self._actors, efforts)

        # Dynamics are calculated by the Isaac Gym simulation
        self._gym.simulate(self._impl)
        self._gym.fetch_results(self._impl, True)
        ...
        if self._curr_step%render_step == 0:
            ...
            if mode.video:
                if self.viewer is None:
                    self.viewer = self._gym.create_viewer(self._impl, self._gym.CameraProperties())

                # Update the viewer
                self._gym.step_graphics(self._impl)
                self._gym.draw_viewer(self.viewer, (self._impl, True))
                self._gym.sync_frame_time(self._impl)  # TODO move this to the rollout function?!
