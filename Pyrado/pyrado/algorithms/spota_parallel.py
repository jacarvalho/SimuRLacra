...
class SPOTA(Algorithm):
    ...
    def _compute_candidate(self, nc: int):
        ...
        self._logger.add_value('refNo', -1)  # RefNo is -1 for the candidate
    ...
    def _compute_references(self, nr: int, nG: int):
        ...
        for k in range(nG):
            ...
            self._logger.add_value('refNo', k)
            ...
            if isinstance(self._subrtn_refs, ActorCritic):
                self._subrtn_refs.policy.train()
    ...
    def _eval_cand_and_ref_one_domain(self, i: int) -> tuple:
        ...
        """
        # Create and initialize a sampler for parallel evaluation
        pool = SamplerPool(6)
        pool.invoke_all(_setup_env_policy_cr, self._env_dr, self._subrtn_cand, self._subrtn_refs,
                        i, self.nJ, self.base_seed)

        # Run with progress bar
        with tqdm(leave=False, file=sys.stdout, unit='rollouts', desc='Evaluating') as pb:
            cand_and_ref_ret_deltas = pool.run_map(_run_rollout_cr, list(range(self.nJ)), pb)

        # Sum over all deltas (i.e. compute the average return)
        cand_and_ref_ret = np.array(cand_and_ref_ret_deltas).sum(axis=0)

        return cand_and_ref_ret[0], cand_and_ref_ret[1]

    def _eval_ref_one_domain(self, ref_policy: Policy, i: int):
        """
        Evaluate the a reference solution (see outer loop) in the i-th domain using nJ rollouts.

        :param ref_policy: reference solution to evaluate (not the current reference)
        :param i: index of the domain to evaluate in
        :return: average return values for the k-th reference in the i-th domain
        """
        # Create a sampler for parallel evaluation
        pool = SamplerPool(4)

        # Initialize pool
        pool.invoke_all(_setup_env_policy_r, self._env_dr, ref_policy, i, self.nJ, self.base_seed)

        # Run with progress bar
        with tqdm(leave=False, file=sys.stdout, unit='rollouts', desc='Evaluating') as pb:
            ref_ret_deltas = pool.run_map(_run_rollout_r, list(range(self.nJ)), pb)

        # Sum over all deltas (i.e. compute the average return)
        ref_ret = np.array(ref_ret_deltas).sum(axis=0)

        return float(ref_ret)
    ...
    def _estimate_ucbog(self, nr: int):
        ...
        .. note:: The dropout and batch normalization layers are set to evaluation mode by the rollout function.
        ...
        np.save(osp.join(self._save_dir, 'iter_{self._curr_iter}_diffs.npy'), self.Gn_diffs)
    ...
    def _handle_neg_samples(self, cand_rets: np.ndarray, refs_rets: np.ndarray, k: int, i: int) -> np.ndarray:
        ...
        if refs_rets[k, i] < cand_rets[k, i]:
            print_cbt(
                    ...
                    f'Trying to replace this reference at this realization with a different one', 'y')
            for other_k in range(self.nG):
                if other_k == k:
                    ...
                    # Do the evaluation
                    other_ref_ret = self._eval_ref_one_domain(other_ref, i)
...
def _setup_env_policy_cr(G, env, cand, ref, i, nJ, base_seed):
    # Assign the values
    G.env = env
    G.cand_policy = cand.policy
    G.ref_policy = ref.policy
    G.i = i
    G.nJ = nJ
    G.base_seed = base_seed


def _run_rollout_cr(G, r):
    """
    Run one rollout of the candidate and the reference solution in the i-th domain with a fixed random seed.
    .. note:: Seed before every rollout.
    .. note:: The order of the rollouts is non-deterministic due to parallel sampling.

    :param G: thread-local global variables
    :param r: current rollout index
    :return: one term of the summation (a.k.a. a delta) for the average candidate and the reference return
    """
    # Seed the candidate solution
    set_seed(G.base_seed + G.i * G.nJ + r)
    # Set the circular index for the particular realization
    G.env.ring_idx = G.i
    # Do the rollout and collect the return
    ro_cand = rollout(G.env, G.cand_policy, eval=True)
    cand_ret_avg_delta = ro_cand.undiscounted_return()

    # Seed the reference solution to the same seed as the candidate solution
    set_seed(G.base_seed + G.i * G.nJ + r)
    # Set the circular index for the particular realization to the same index as the candidate solution
    G.env.ring_idx = G.i
    # Do the rollout and collect the return
    ro_ref = rollout(G.env, G.ref_policy, eval=True)
    refs_ret_avg_delta = ro_ref.undiscounted_return()

    return cand_ret_avg_delta / G.nJ, refs_ret_avg_delta / G.nJ  # average over nJ seeds


def _setup_env_policy_r(G, env, ref_policy, i, nJ, base_seed):
    # Assign the values
    G.env = env
    G.ref_policy = ref_policy
    G.i = i
    G.nJ = nJ
    G.base_seed = base_seed


def _run_rollout_r(G, r):
    """
    Run one rollout of the reference solution in the i-th domain with a fixed random seed.
    .. note:: Seed before every rollout.
    .. note:: The order of the rollouts is non-deterministic due to parallel sampling.
    ...
    # Do the rollout and collect the return
    ro_ref = rollout(G.env, G.ref_policy, eval=True)
    return ro_ref.undiscounted_return() / G.nJ  # average over nJ seeds
