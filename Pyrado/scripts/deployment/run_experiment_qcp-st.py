...
def experiment_wo_distruber(env_real: RealEnv):
    # Wrap the env in the same as done during training
...
def experiment_w_distruber(env_real: RealEnv):
    # Wrap the env in the same as done during training
    env_real = wrap_like_other_env(env_real, env_sim)

    # Run learned policy on the device
    print_cbt('Running the evaluation policy ...', 'c')
    ro1 = rollout(env_real, policy, eval=True, max_steps=int(args.max_steps/3), render_mode=RenderMode(),
                  no_reset=True, no_close=True)
    ...
    # Run learned policy on the device
    print_cbt('Running the evaluation policy ...', 'c')
    ro2 = rollout(env_real, policy, eval=True, max_steps=int(args.max_steps/3), render_mode=RenderMode(),
                  no_reset=True, no_close=True)
    ...
    # Run learned policy on the device
    print_cbt('Running the evaluation policy ...', 'c')
    ro3 = rollout(env_real, policy, eval=True, max_steps=int(args.max_steps/3), render_mode=RenderMode(),
                  no_reset=True, no_close=True)
...
if __name__ == '__main__':
    ...
    disturber_pos = TimePolicy(env.spec, volt_disturbance_pos, env.dt)
    disturber_neg = TimePolicy(env.spec, volt_disturbance_neg, env.dt)
    steps_disturb = 10
    ...
    for r in range(args.num_runs):
        ro = experiment_wo_distruber(env)
        # ro = experiment_w_distruber(env)
        ros.append(ro)
