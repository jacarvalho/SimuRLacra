...
protected:
    ...
    virtual ObservationModel* createObservationModel()
    {
        ...
        if (properties->getPropertyBool("observeVelocities", true))
        {
            auto omBoxLin = new OMBodyStateLinear(graph, "Box", "GoalUpperShelve", "GoalUpperShelve");  // in relative coordinates
        ...
        {
            auto omBoxLin = new OMBodyStateLinearPositions(graph, "Box", "GoalUpperShelve", "GoalUpperShelve");  // in relative coordinates
            omBoxLin->setMinState({-0.2, -0.4, -0.4});  // [m]
            omBoxLin->setMaxState({1.8, 0.4, 1.0});  // [m]
            fullState->addPart(omBoxLin);
        }
