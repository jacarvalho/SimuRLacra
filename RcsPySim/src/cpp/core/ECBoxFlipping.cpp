...
//            amIK->addTask(new TaskDistance1D(graph, rightCP, box, 1));  // Y
    ...
    virtual ObservationModel* createObservationModel()
    {
        ...
        if (properties->getPropertyBool("observeTaskSpaceDiscrepancy", true))
        {
            ...
            if (wamIK)
            {
                auto omTSDescrL = new OMTaskSpaceDiscrepancy("ContactPoint_L", graph, wamIK->getController()->getGraph());
                fullState->addPart(OMPartial::fromMask(omTSDescrL, {false, true, true}));
                auto omTSDescrR = new OMTaskSpaceDiscrepancy("ContactPoint_R", graph, wamIK->getController()->getGraph());
...
public:
        ...
        ForceDisturber* forceDisturber) override
    {
        ...
        linesOut.emplace_back(
            string_format("num joints:    %d total, %d pos ctrl, %d trq ctrl",
